...
def _build_app(resolver: MagicMock, repo: MagicMock) -> TestClient:
...
def _teardown(tc: TestClient, patcher) -> None:
...
class TestHealthz:
    ...
    def test_returns_200_with_status_ok(self) -> None:
        ...
        repo = _mock_repo()
        tc, patcher = _build_app(resolver, repo)
        try:
            response = tc.get("/healthz")
        finally:
            _teardown(tc, patcher)
