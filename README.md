...
**詳細な運用手順は [docs/runbooks/reindex.md](docs/runbooks/reindex.md) を参照** (各シナリオに前提条件・期待ログ・失敗時アクション・チェックリストを掲載)。

### 初回カットオーバー (1 回限り、停止許容)

本機能をローカル環境に初めて導入する際は、既存の `artworks_v1` コレクションに対して一度だけエイリアスを張る手順が必要。この初回カットオーバーは**検索サービスの停止を許容する** (バッチインジェスション進行中でないことを確認すれば数秒の切替で完了):

```bash
# 1. 検索サービスを停止 (alias 未定義で起動失敗を避ける)
docker compose stop search

# 2. エイリアス artworks_current を artworks_v1 に張る
docker compose run --rm ingestion python -m services.ingestion.reindex init-alias

# 3. 検索サービスを再起動 (これ以降 /readyz が 200 を返す)
docker compose up -d search
```

### 日常の無停止再インデックス

初回カットオーバー以降は**以下の流れで検索サービスを止めずに再インデックスできる**:
...
# 2. 本番再インデックス: 新物理コレクション作成 → populate → 検証 → alias 切替
docker compose run --rm ingestion python -m services.ingestion.reindex run \
    --target-version v2

# 3. (必要なら) 再インデックス中に旧へ入った差分を新へキャッチアップ
...
# 5. (観察期間経過後) 旧コレクションを削除
docker compose run --rm ingestion python -m services.ingestion.reindex drop-collection artworks_v1
```

切替は Qdrant エイリアスをアトミックに更新するため、検索リクエストは停止せず新コレクションへルーティングされる。検証失敗時は切替を行わず、旧コレクションが稼働を継続する。

各手順の前提条件・期待ログ・失敗時アクション・成功判定チェックリストは [docs/runbooks/reindex.md](docs/runbooks/reindex.md) を参照。
